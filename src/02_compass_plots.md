...
This notebook generates compass plots and analyses directional patterns
in the formulas. To fully reproduce Figures 3 and 4, load the direction
plots from the `03_directions_plot.qmd` into the R environment.

-   Calculate Coordinates: Transforms ‘from’ coordinates to (0,0) and
    adjusts ‘to’ coordinates accordingly;

-   Long Distances: Creates six compass plots showing long distances
    directions across corpora;

-   Short Distances: Produces same kind of plots for short-distance
    formulas;

-   Figure 5: Prepares background map and compasses used in the Fig. 5.

-   Additional - Rivers: Generate compass plots for additional river
    coordinates to examine their directional distribution.
...
Load data & calculate distances
    ...
    # A tibble: 5 × 5
      ...
      <chr> <chr>                         <chr>          <chr>                 <dbl>
    1 de    von Portugal bis China        Portugal       People's Re…         9234. 
    2 en    From Feale Water to Kenmare   River Feale    Kenmare                67.5
    3 en    From Beachy Head to Skiddaw   Beachy Head    Skiddaw               492. 
    4 fr    de Mayence à Grenade          Mainz          Granada              1714. 
    5 en    from peaks of Pyrenees To Gr… Pyrenees       Graian Alps           585. 
...
``` r
# the function creates the plot for directions distribution (long distances), same as in 03_directions_plot.qmd; it first assign each formula to a segment, and then calculate proportion of formulas in each segment.
